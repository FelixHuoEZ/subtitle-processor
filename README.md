...
1. Generate and push images from a build machine:
   ...
   - To avoid unstable Docker Hub access on the build machine, set `BASE_IMAGE_REGISTRY` to an internal mirror prefix such as `registry.example.com:5443/dockerhub`. The Dockerfiles will then read:
...
1. 在构建机器上生成并推送镜像：
   ...
   - 如果构建机访问 Docker Hub 不稳定，可以设置 `BASE_IMAGE_REGISTRY=registry.example.com:5443/dockerhub` 之类的内网 mirror。各 Dockerfile 会改为读取：
     - `${BASE_IMAGE_REGISTRY}/library/python:3.11-slim`
     - `${BASE_IMAGE_REGISTRY}/library/python:3.9-slim`
     - `${BASE_IMAGE_REGISTRY}/nvidia/cuda:11.8.0-base-ubuntu22.04`
     - `${BASE_IMAGE_REGISTRY}/brainicism/bgutil-ytdlp-pot-provider:1.2.2`
